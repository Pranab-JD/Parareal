...
int main(int argc, char** argv)
{
    ...
    double *device_u; cudaMalloc(&device_u, N_size);                    //* Initial vector
    cudaMemcpy(device_u, &u_init[0], N_size, cudaMemcpyHostToDevice);   //* Copy state variable to device
    double *device_u_sol; cudaMalloc(&device_u_sol, N_size);            //* Solution vector
    double *device_u_temp;                                              //* Temporary vector(s) for integrators
    ...
    else if (integrator == "RK2")
    {
        ...
        cudaMalloc(&device_u_temp, 2*N_size);
    }
    else if (integrator == "RK4")
    {
        // u_temp = (double*)malloc(4*N_size);
        cudaMalloc(&device_u_temp, 4*N_size);
    }
    else
    {
    ...
    for (int nn = 0; nn < num_time_steps; nn++)
    {
        ...
        if (integrator == "Explicit_Euler")
        {
            explicit_Euler(RHS, device_u, device_u_sol, device_u_temp, dt, N, GPU_access);
        }
        else if (integrator == "RK2")
        {
            RK2(RHS, device_u, device_u_sol, device_u_temp, dt, N, GPU_access);
        }
        else if (integrator == "RK4")
        {
            RK4(RHS, device_u, device_u_sol, device_u_temp, dt, N, GPU_access);
        }
        else
        {
            cout << "Incorrect integrator! Please recheck. Terminating simulations ... " << endl << endl;
        ...
        //TODO: Currently NOT supported for GPU code
        if (time_steps % output_cycle == 0 && output_cycle > 0)
        {
            ...
            for(int ii = 0; ii < N; ii++)
            {
                data << setprecision(16) << u_init[ii] << endl;
            }
            data.close();
        }
    }
    ...
    cudaMemcpy(&u_init[0], device_u, N_size, cudaMemcpyDeviceToHost);   
    ...
    int sys_value = system(("mkdir -p ./" + integrator + "/GPU").c_str());
    string directory = "./" + integrator + "/GPU";
